...
static void *drawtextscanline8(uint8_t *scanline, int w, int y, uint32_t *clr,
			       unsigned char const *text, int len)
{
	...
	for (x = 0 ; w && x < sdlg.font.w[text[n]] ; ++x, --w)
	    scanline[x] = (uint8_t)clr[glyph[x]];
	scanline += x;
    }
    while (w--)
	*scanline++ = (uint8_t)clr[0];
    return scanline;
}

static void *drawtextscanline16(Uint16 *scanline, int w, int y, uint32_t *clr,
				unsigned char const *text, int len)
{
    unsigned char const	       *glyph;
    int				n, x;
...
static void *drawtextscanline24(uint8_t *scanline, int w, int y, uint32_t *clr,
				unsigned char const *text, int len)
{
    unsigned char const	       *glyph;
    uint32_t			c;
	...
	for (x = 0 ; w && x < sdlg.font.w[text[n]] ; ++x, --w) {
	    ...
	    *scanline++ = (uint8_t)(c >> 16);
	    *scanline++ = (uint8_t)(c >> 8);
	    *scanline++ = (uint8_t)c;
#else
	    *scanline++ = (uint8_t)c;
	    *scanline++ = (uint8_t)(c >> 8);
...
static void *drawtextscanline32(uint32_t *scanline, int w, int y, uint32_t *clr,
				unsigned char const *text, int len)
{
    unsigned char const	       *glyph;
    int				n, x;
