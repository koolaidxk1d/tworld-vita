...
static int keyinputcallback(void)
{
    ...
    switch (ch) {
	  case CmdNorth: 
	    if (passwd[selectedCharIndex] == 'Z') {
			passwd[selectedCharIndex] = 'A';
		} else {
			passwd[selectedCharIndex] += 1;
		} 
	  case CmdSouth: 
	    if (passwd[selectedCharIndex] == 'A') {
			passwd[selectedCharIndex] = 'Z';
		} else {
			passwd[selectedCharIndex] -= 1;
		} 
      case CmdWest:
...
{
    passwd[0], passwd[1], passwd[2], passwd[3] = "A", "A", "A", "A";
	selectedCharIndex = 0;
    int		n;
				...
				char const *defaultseries, int defaultlevel)
{
	...
	char* captionf = malloc(256);
	sprintf(captionf, "   Welcome to Tile World. Press Start for help. Save dir is %s", getsavedir());

	for (;;) {
	    f = displaylist(captionf,
...
static int getsettingsfromcmdline(int argc, char *argv[], startupdata *start)
{
    ...
    *start->filename = '\0';
    ...
    start->savedir = "ux0:/data/tworld/save";
