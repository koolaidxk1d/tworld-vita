 ...
 */
static uint32_t pixelat(SDL_Surface *s, int x, int y)
{
    switch (s->format->BytesPerPixel) {
      case 1:
	return (uint32_t)((uint8_t*)s->pixels + y * s->pitch)[x];
      case 2:
	return (uint32_t)((Uint16*)((uint8_t*)s->pixels + y * s->pitch))[x];
      case 3:
#if SDL_BYTEORDER == SDL_BIG_ENDIAN
	return (uint32_t)((((uint8_t*)s->pixels + y * s->pitch)[x * 3 + 0] << 16)
		      | (((uint8_t*)s->pixels + y * s->pitch)[x * 3 + 1] << 8)
		      | (((uint8_t*)s->pixels + y * s->pitch)[x * 3 + 2] << 0));
#else
	return (uint32_t)((((uint8_t*)s->pixels + y * s->pitch)[x * 3 + 0] << 0)
		      | (((uint8_t*)s->pixels + y * s->pitch)[x * 3 + 1] << 8)
		      | (((uint8_t*)s->pixels + y * s->pitch)[x * 3 + 2] << 16));
#endif
      case 4:
	return ((uint32_t*)((uint8_t*)s->pixels + y * s->pitch))[x];
    }
